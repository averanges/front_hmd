...
import 'package:haimdall/src/presentation/farmland/home/record/drain_water/drain_water_record_controller.dart';
import 'package:intl/intl.dart';

class DrainWaterRecordPage extends GetView<DrainWaterRecordController> {
  const DrainWaterRecordPage({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<DrainWaterRecordController>(
      init: DrainWaterRecordController(
        farmlandId: Get.arguments['farmlandId'],
        wateringId: Get.arguments['wateringId'],
        repository: Get.find(),
      ),
  ...
  Widget _titleView() => SizedBox(
        ...
        child: Stack(
          children: [
            ...
            TitleView(title: 'record_drain_water_title'.tr),
          ],
        ),
      );
  ...
  Widget _infoView() => Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 25.0),
            _recordTimeView(controller.recordTime.value),
            const SizedBox(height: 10.0),
            _rowGPSInfo(
                'latitude'.tr, controller.location.value?.latitude ?? 0.0),
            const SizedBox(height: 10.0),
            _rowGPSInfo(
                'longitude'.tr, controller.location.value?.longitude ?? 0.0),
          ],
        ),
      );

  Widget _recordTimeView(DateTime time) => SizedBox(
        height: 33.0,
        child: Row(
          children: [
            Text(
            ...
            Expanded(
              child: Container(
                ...
                child: Text(
                  DateFormat('yyyyMMddhhmm').format(time),
                  style: const TextStyle(
                    color: AppColors.textLabel1st,
